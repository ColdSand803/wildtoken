...
// AdminListModelTestTemplates returns the request wrappers used by model tests.
func AdminListModelTestTemplates(state *appstate.State) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		templates, err := db.ListModelTestTemplates(r.Context(), state.DB)
		if err != nil {
			apperr.WriteError(w, err)
			return
		}
...
func AdminCreateModelTestTemplate(state *appstate.State) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		template, err := db.CreateModelTestTemplate(r.Context(), state.DB, &input)
		if err != nil {
			if isUniqueViolation(err) {
				apperr.WriteError(w, apperr.BadRequest("template name already exists"))
				return
			}
			apperr.WriteError(w, err)
			return
...
func AdminUpdateModelTestTemplate(state *appstate.State) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		id, err := pathID(r)
		if err != nil {
			apperr.WriteError(w, err)
		...
		template, found, err := db.UpdateModelTestTemplate(r.Context(), state.DB, id, &input)
		if err != nil {
			apperr.WriteError(w, err)
			return
		}
		if !found {
			apperr.WriteError(w, apperr.NotFound("model test template not found"))
			return
		}
...
func AdminDeleteModelTestTemplate(state *appstate.State) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		id, err := pathID(r)
		if err != nil {
			apperr.WriteError(w, err)
		...
		deleted, err := db.DeleteModelTestTemplate(r.Context(), state.DB, id)
		if err != nil {
			apperr.WriteError(w, err)
			return
		}
