import { APPEARANCE_EVENT, currentTheme } from "./theme";
...
export function mountPlum(plumCanvas: HTMLCanvasElement): () => void {
  const context = plumCanvas.getContext("2d");
  const parent = plumCanvas.parentElement;
  if (!context || !parent) return () => {};
  const plumCtx = context;
  const layer = parent;
  let plumFrameId: number | null = null;
  let plumRestartTimer = 0;
  let plumSteps: Array<() => void> = [];
  let plumDrawn = 0, plumWidth = 0, plumHeight = 0, plumLastFrame = 0;
...
function plumPolarToCart(x: number, y: number, radius: number, theta: number) {
  return [x + radius * Math.cos(theta), y + radius * Math.sin(theta)];
}

/* 一步 = 画一段短线，然后按概率分出左右两枝。counter 沿整棵子树共享并累加，所以
   "头 30 段"数的是这一根的深度，不是全画布的总数。 */
function plumStep(x: number, y: number, rad: number, counter: { value: number }) {
...
function plumScheduleRegrow() {
  ...
  const sync = () => {
    plumStopFrames(); clearTimeout(plumRestartTimer); plumSteps = [];
    layer.hidden = !PLUM_THEMES.has(currentTheme());
    if (!layer.hidden && !document.hidden) plumGrow();
    else plumCtx.clearRect(0, 0, plumWidth, plumHeight);
  };
  ...
  return () => {
    ...
    window.removeEventListener("resize", plumScheduleRegrow);
