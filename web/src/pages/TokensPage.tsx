...
import { QUOTA_PERIOD_LABELS } from "../tokenPolicy";
...
function TokenRow({
  ...
  return (
    <tr className={token.enabled ? undefined : "row-disabled"}>
      ...
      <td><div className="token-models-cell" title={(token.allowed_models ?? []).join(", ")}>{token.allowed_models?.length ? <>{token.allowed_models.slice(0, 2).map((model) => <span className="badge token-model-tag" key={model}>{model}</span>)}{token.allowed_models.length > 2 && <span className="badge">+{token.allowed_models.length - 2}</span>}</> : <span className="muted">全部模型</span>}</div></td>

      <td className="col-quota">
        <QuotaCell token={token} />
        {token.quota_period_state?.period !== "none" && token.quota_period_state && <small className="quota-rate-note" title={token.quota_period_state.timezone}>{QUOTA_PERIOD_LABELS[token.quota_period_state.period]}重置{token.quota_period_state.next_reset_at ? ` · 下次 ${new Date(token.quota_period_state.next_reset_at).toLocaleString("zh-CN")}` : ""}</small>}
      </td>
