    ...
    <section className="view settings-view" data-view="settings">
      <section className="panel settings-hero">
        ...
        <div className="settings-stack">
              ...
              <section className="settings-card">
                ...
                <div className="routing-rule-guide">
                  <ol className="routing-rule-steps">
                    ...
                    <li>
                      <strong>同层按策略选择</strong>
                      <span>选择有效权重随机或最低延迟优先；冷启动样本不足时仍按有效权重分流。</span>
                    </li>
                    <li>
                  ...
                  <p className="routing-rule-detail">
                    ...
                    显式指定渠道的重试始终等待。有可用重试时先等 SSE 首个有效事件再提交响应；提交后不再透明重试，避免重复输出。
                  </p>
                </div>

                <div className="settings-server-form">
                  <div className="settings-fields-grid routing-settings-grid">
                    <label className="field span-2"><span className="field-label">负载均衡策略</span><select id="setting-load-balance-strategy" value={settings.load_balance_strategy ?? "weighted"} onChange={(event) => patch("load_balance_strategy", event.target.value as RuntimeSettings["load_balance_strategy"])}><option value="weighted">有效权重随机</option><option value="least_latency">最低延迟优先</option></select><span className="field-hint">仅在同优先级内比较；样本不足时回退到有效权重。</span></label>
                    <NumberField
                      label="最大重试次数"
          ...
          <DataManagement onChanged={reload} />
          <section className="settings-card metrics-endpoint-card">
            <div className="settings-card-head"><div><h3>Prometheus 监控端点</h3><p>由启动配置管理，只读，不显示访问凭证。</p></div></div>
            {system?.metrics_endpoint ? <p>{system.metrics_endpoint.enabled ? "已启用" : "已停用"} · <code>{system.metrics_endpoint.path}</code> · {system.metrics_endpoint.token_required ? "需要访问令牌" : "未设置访问令牌，请限制网络访问"}</p> : <p>端点状态暂不可用。</p>}
          </section>
          <section className="settings-card">
            <div className="settings-card-head">
              <div>
