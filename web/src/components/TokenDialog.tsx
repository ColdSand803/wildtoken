...
import { parseAllowedModelsInput, QUOTA_PERIOD_LABELS } from "../tokenPolicy";
...
export function TokenDialog({
  ...
  useEffect(() => {
    ...
    setQuotaTimezone(token?.quota_period_state?.timezone ?? "Asia/Shanghai");
    // 编辑时把现有到期时间填回输入框，保存时原样解回去，不动就不会变。
    setExpires(expiryInputValue(token?.expires_at ?? null));
  }, [open, token, groups]);

  /* 边输边算，看得见结果才敢填 1d3h 这种写法。 */
  const parsedModels = parseAllowedModelsInput(allowedModels);
    ...
    <dialog className="upstream-dialog dialog--drawer" ref={ref} onCancel={onClose}>
      <form
        ...
        onSubmit={(event) => {
          ...
          if (busy || !name.trim() || !parsedExpiry.ok || !parsedModels.ok) return;
          onSubmit({
            name: name.trim(),
            ...
            allowed_models: parsedModels.allowedModels,
            quota_period: quotaPeriod,
            quota_timezone: quotaTimezone.trim(),
      ...
      >
        ...
        <div className="upstream-dialog-body">
            ...
            <div className="form-grid">
              <label className="field">
                ...
                <span className="field-hint">
                  按当前配额周期的 token 总量计算，支持 K/M/B/T 后缀。
                </span>
              </label>
          ...
          <section className="form-section">
            <div className="form-section-head"><h3>模型与配额周期</h3></div>
            <div className="form-grid">
              <label className="field span-2"><span className="field-label">允许模型</span>
                <textarea id="token-allowed-models" value={allowedModels} onChange={(event) => setAllowedModels(event.target.value)} rows={3} placeholder="留空允许全部；换行或逗号分隔，支持 claude-*" />
                <span className={parsedModels.ok ? "field-hint" : "field-hint field-hint-error"}>{parsedModels.ok ? "按客户端请求的模型名匹配，最多 200 项。" : parsedModels.error}</span>
              </label>
              <label className="field"><span className="field-label">配额重置周期</span>
                <select id="token-quota-period" value={quotaPeriod} onChange={(event) => setQuotaPeriod(event.target.value)}>{Object.entries(QUOTA_PERIOD_LABELS).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select>
              </label>
              <label className="field"><span className="field-label">配额时区</span>
                <input id="token-quota-timezone" value={quotaTimezone} onChange={(event) => setQuotaTimezone(event.target.value)} placeholder="Asia/Shanghai" required />
                <span className="field-hint">IANA 时区；周一／每月一日当地零点重置。</span>
              </label>
            </div>
          </section>
          <section className="form-section">
        ...
        <div className="modal-footer">
          ...
          <button type="submit" className="primary" disabled={busy || !name.trim() || !parsedExpiry.ok || !parsedModels.ok}>
            {busy ? "保存中…" : "保存"}
          </button>
        </div>
      </form>
