...
export interface BarSegment { label: string; width: number; className?: string; lines?: string[]; onSelect?: () => void }
...
export function SegmentBar({ segments, className = "", label }: { segments: BarSegment[]; className?: string; label: string }) {
  ...
  return <div ref={host} className="wt-hovercard-host" onMouseLeave={() => setHovered(null)} onBlur={(event) => { if (!event.currentTarget.contains(event.relatedTarget)) setHovered(null); }} onKeyDown={(event) => { if (event.key === "Escape") setHovered(null); }}>
    <div className={`wt-segbar ${className}`} role="group" aria-label={label}>{segments.filter((segment) => segment.width > 0).map((segment) => {
      const index = segments.indexOf(segment);
      return <span key={index} className={`wt-segbar-seg is-hoverable ${segment.className ?? ""}`} style={{ width: `${Math.max(0, segment.width)}%` }} role={segment.onSelect ? "button" : "img"} tabIndex={0} aria-label={[segment.label, ...(segment.lines ?? [])].join("；")} aria-describedby={hovered?.index === index ? tooltipId : undefined}
