...
管理界面内置主题仍从 `static/css/*.css` 加载；额外主题可以作为 CSS-only 主题包放在 `themes/` 下，Docker Compose 会把宿主机 `./themes` 只读挂载到容器的 `/app/themes`。Bleach 主题随仓库以 `themes/bleach` 主题包形式提供，不再内嵌到主样式表。
